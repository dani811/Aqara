...
`aqara_u200_ble/transport.py` (the low-level radio layer) imports
`AUTH_SERVICE_UUID`, `CONTROL_SERVICE_UUID`, `AUX_SERVICE_UUID` from
`aqara_u200_ble/session.py` (the top authenticated-protocol/orchestration layer),
...
(`aqara_u200_ble/gatt_uuids.py`) with zero internal imports, and has both
...
upward from `session`. Import `aqara_u200_ble.gatt_uuids` and confirm it pulls in
...
2. **Given** the leaf module, **When** it is imported in isolation, **Then** it
   imports no other `aqara_u200_ble` module (it is a true leaf).
...
`from aqara_u200_ble import <name>` works for all moved constants; the full test
...
- **FR-001**: A new leaf module `aqara_u200_ble/gatt_uuids.py` MUST hold the GATT
...
- **FR-006**: The public `__all__` in `__init__.py` MUST export exactly the same
  ...
  `from aqara_u200_ble import <name>`).
...
- **Leaf module**: `aqara_u200_ble/gatt_uuids.py` — the new zero-dependency home
  for those constants.
