...
Mover el runner a `aqara_ble/cli.py` (adaptador fino), registrar el *console
script* `aqara` en `pyproject.toml`, y garantizar que `import aqara_ble` no
carga el CLI ni lee el entorno. `examples/lock_cli.py` pasa a un envoltorio de una
línea que delega en `aqara_ble.cli:main`. Ningún byte de protocolo cambia; la
...
preserva la decisión de 014 con un test de que `import aqara_ble` no importa
...
| --- | --- | --- |
| I Secretos / pureza | ✅ | `cli.py` (consumidor) lee entorno; la API importable no. Test: `import aqara_ble` no carga `cli` ni toca `os.environ`. Sin secretos impresos. |
...
pyproject.toml       # [project.scripts] aqara = "aqara_ble.cli:main"; cli.py fuera del extend-exclude
examples/
├── lock_cli.py      # queda como envoltorio: `from aqara_ble.cli import main` (compat)
...
por `__init__`, así el comando se instala pero `import aqara_ble` sigue puro.
