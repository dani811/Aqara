...
- [X] T004 Correr `pytest`, `ruff`, `mypy aqara_u200_ble` en verde.
...
- [X] T006 [US3] Gate de release: `pytest`, `ruff check`, `ruff format --check`, `mypy --strict`; `python -m build`; verificar que la wheel contiene `aqara_u200_ble/` y `py.typed` e instala en un venv limpio.
...
- [ ] T009 Cerrar issues con referencia al commit/tag: #1 (resuelta por 012), #2, #3 (esta feature), #4 (release 0.5.0). Comentar en el PR #2 de `haos_aqara` que ya puede fijar `aqara-u200-ble==0.5.0`.
