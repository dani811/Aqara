...
`aqara_ble/kdf.py` (842 LOC) was misnamed and mixed two concerns: pure,
...
`aqara_ble/cloud_crypto.py` (no network, no `kdf` import). `kdf.py` becomes
...
**Independent Test**: `cloud_crypto.py` imports no other `aqara_ble` module
...
- **FR-001**: A new module `aqara_ble/cloud_crypto.py` MUST contain the pure
...
- **FR-002**: `cloud_crypto.py` MUST import no other `aqara_ble` module and
...
- **FR-005**: The public `__all__` MUST export exactly the same names as before,
  reachable via `from aqara_ble import <name>`.
...
- **kdf.py**: the cloud HTTP client / orchestration layer (kept name to avoid
  breaking `from aqara_ble.kdf import ...`).

## Success Criteria *(mandatory)*

### Measurable Outcomes

- **SC-001**: `cloud_crypto.py` contains 0 imports of any other `aqara_ble`
...
- **SC-004**: `aqara_ble.__all__` is identical before and after.
...
- Keeping the module name `kdf.py` (rather than renaming to `cloud_http.py`) is the
  right risk/value trade-off: it preserves every `from aqara_ble.kdf import
