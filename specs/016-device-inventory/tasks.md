...
- [X] T001 [US1] `aqara_u200_ble/models.py`: `MODEL_BY_PRODUCT_ID={0x9C03:"U200"}`, `decode_manufacturer_payload(payload)->(product_id|None, model|None)` (u16 LE offset 2; product_id sin bytes → None; model no catalogado → None) + docstring con la evidencia.
