...
| Tool | What it does |
| --- | --- |
| [`esp32s3_hci_usb/`](esp32s3_hci_usb/README.md) | **Firmware** that turns an ESP32‑S3 into a BLE HCI controller over its native USB port (H4 over USB‑Serial‑JTAG). Source + build/erase/flash recipe; no binaries. This is what `aqara_ble.BumbleTransport` drives. |
